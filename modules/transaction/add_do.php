...
if(isset($_POST["transaction_add"])){
	...
	if($err==""){
		$sql="INSERT INTO transaction ( wing_id, account_id, reference_id, code,date_added, amount, details, cheque_number) VALUES ( '".slash($wing_id)."', '".slash($account_id)."','".slash($reference_id)."','".slash($code)."','".slash($date_added)."','".slash($amount)."','".slash($details)."','".slash($cheque_number)."')";
		doquery($sql,$dblink);
