...
if(isset($_POST["transaction_edit"])){
	...
	if($err==""){
		$sql="Update transaction set `wing_id`='".slash($wing_id)."',`account_id`='".slash($account_id)."',`reference_id`='".slash($reference_id)."',`code`='".slash($code)."',`datetime_added`='".date_dbconvert(slash($datetime_added))."',`amount`='".slash($amount)."',`details`='".slash($details)."',`cheque_number`='".slash($cheque_number)."'"." where id='".$id."'";
		doquery($sql,$dblink);
