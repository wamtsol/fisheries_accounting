...
if(isset($_POST["expense_add"])){
	...
	if($err==""){
		$sql="INSERT INTO expense (wing_id, datetime_added, voucher_no, major_head, sub_head, payee, details, amount, income_tax, income_tax_deducted, cheque_amount, cheque_date, cheque_number, added_by) VALUES ('".slash($wing_id)."', '".slash(datetime_dbconvert($datetime_added))."', '".slash($voucher_no)."', '".slash($major_head)."', '".slash($sub_head)."', '".slash($payee)."', '".slash($details)."','".slash($amount)."', '".slash($income_tax)."', '".slash($income_tax_deducted)."', '".slash($cheque_amount)."', '".date_dbconvert($cheque_date)."', '".slash($cheque_number)."','".$_SESSION["logged_in_admin"]["id"]."')";
		doquery($sql,$dblink);
